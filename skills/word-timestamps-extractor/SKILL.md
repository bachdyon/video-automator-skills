...
- Tên trường (`audio_path`, `sentence_id`, `confidence`...), tên model (`large-v3`, `whisper-1`...), CLI flag, file path giữ nguyên tiếng Anh — không dịch.
...
2. Dùng local `faster-whisper` mặc định để lấy word timestamps nhanh hơn API.
3. Chỉ dùng OpenAI Whisper bằng `--backend openai` khi user yêu cầu cloud/API hoặc local thiếu tài nguyên.
