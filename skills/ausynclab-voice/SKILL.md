...
description: Làm việc với AusyncLab voice và Text-to-Speech API khi có `AUSYNCLAB_API_KEY`: liệt kê voice, gợi ý voice cho creative plan, lưu setting voice ưa thích, sinh audio narration mp3 hoặc wav, và hậu xử lý tốc độ WAV sau TTS bằng pydub. Nếu không có `AUSYNCLAB_API_KEY`, dùng `$free-tts`.
---
...
Trước khi chạy bất kỳ script nào của skill này, đọc file `.env` ở repo-root trước. File này nằm cạnh `jobs/`, `skills/`, và `env.example`. Xác nhận `AUSYNCLAB_API_KEY` tồn tại. Nếu thiếu hoặc rỗng, không chạy AusyncLab; chuyển sang `$free-tts` để dùng VieNeu-TTS local/free. Khi synthesize bằng AusyncLab, xác nhận `AUSYNCLAB_VOICE_ID` hoặc `--voice-id`. Truyền `.env` qua `--env-file`; tuyệt đối không in giá trị secret. Chỉ dùng `--env-file` không phải repo-root khi user yêu cầu rõ ràng.
...
Các endpoint/base URL đã được quản lý tập trung trong script `skills/ausynclab-voice/scripts/ausynclab_voice.py` (`VOICE_BASE`, `SPEECH_BASE`).
...
```bash
python skills/ausynclab-voice/scripts/ausynclab_voice.py --env-file .env list --output source/voices.toml
python skills/ausynclab-voice/scripts/ausynclab_voice.py --env-file .env recommend --language vi --use-case NARRATION --save-preference
python skills/ausynclab-voice/scripts/ausynclab_voice.py --env-file .env synthesize --creative-plan source/creative_plan.toml
```

Cho job-scoped run:

```bash
python skills/ausynclab-voice/scripts/ausynclab_voice.py --env-file .env synthesize \
...
```bash
pip install -r skills/ausynclab-voice/scripts/requirements-voice-speed.txt
```
...
```bash
python skills/ausynclab-voice/scripts/ausynclab_voice.py speed-pydub \
...
```bash
python skills/ausynclab-voice/scripts/voice_speed_pydub.py \
  --input jobs/<job_id>/source/voice.wav \
