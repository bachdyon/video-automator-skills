...
3. **Voiceover Approval Gate (BẮT BUỘC)**
   - Input: kịch bản voiceover từ `creative_plan.toml` hoặc `source/narration.txt`.
   - Agent phải gửi nguyên văn voiceover script cho user và chờ user duyệt rõ ràng.
   - Output sau khi duyệt: `jobs/<job_id>/source/voiceover_approval.toml` với `approved = true`, `approved_at`, và `script_path`.
   - Không chạy voice/TTS, transcript, AI image synthesis, asset mapping, render plan, hoặc render trước khi gate này pass.
   - Nếu user sửa script, cập nhật script và quay lại gate này; mọi audio/transcript/render phụ thuộc script cũ phải đánh dấu stale.
...
4. **Voice**
   ...
   - **Tùy chọn (theo yêu cầu user): tăng/giảm tốc sau TTS** — không gọi lại AusyncLab: dùng `speed-pydub` trong `skills/ausynclab-voice/scripts/ausynclab_voice.py` hoặc `voice_speed_pydub.py` (pydub `speedup`). Cài `skills/ausynclab-voice/scripts/requirements-voice-speed.txt` nếu môi trường thiếu `pydub` / trên Python 3.13+ thêm `audioop-lts`. Sau khi ghi đè `voice.wav`, **bắt buộc** đồng bộ độ dài intro với audio mới: cập nhật `source/render_plan.toml` + `remotion/src/Root.tsx`, copy `voice.wav` → `remotion/public/assets/`, chạy lại `$word-timestamps-extractor` nếu cần transcript khớp waveform mới, rồi các bước phụ thuộc transcript/render plan nếu có.
...
6. **AI Image Synthesis (CÓ ĐIỀU KIỆN)**
...
8. **Semantic Mapping (baseline 1-1)**
...
9. **Shot Coverage Decisions (sáng tạo)**
...
11. **Render**
   ...
   - Khi bàn giao, phản hồi cuối bắt buộc nhúng video trong Codex bằng Markdown media tag với path tuyệt đối, theo quy tắc bàn giao của `video-renderer`.
...
- `source/voiceover_approval.toml` tồn tại và `approved = true` trước mọi bước voice/TTS hoặc render phụ thuộc script.
...
- Phản hồi bàn giao cuối có nhúng video bằng Markdown media tag với path tuyệt đối, ví dụ `![Final video](/absolute/path/to/jobs/<job_id>/output/final_video.mp4)`, để user bấm xem ngay trong Codex.
