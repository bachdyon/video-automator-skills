...
- Khi tạo job, đặt `--title` bằng tiếng Việt có dấu để lưu trong `job.toml`; phần tên trong `job.id`/directory phải là tiếng Việt không dấu, lowercase, nối bằng gạch ngang. Script `create` tự bỏ dấu từ `--title` (vd `Tư duy làm giàu` -> `2026-04-25_001_tu-duy-lam-giau`).
- Nếu truyền `--job-id` thủ công, bắt buộc dùng tiếng Việt không dấu/lowercase với ký tự `[a-z0-9_-]`; không dùng dấu tiếng Việt, khoảng trắng, chữ hoa, emoji, hoặc tên tiếng Anh cho phần mô tả job.
...
## Remotion dependency reuse

Khi tạo job mới, `remotion/node_modules` phải là symlink về repo root `../../../node_modules`. Không chạy `npm install` trong từng job; nếu thiếu dependency, chạy `npm install` một lần ở repo root.
...
```bash
python skills/video-job-manager/scripts/manage_job.py create \
  --title "Quảng cáo thói quen buổi sáng" \
  --brief "Tạo TikTok 45s phong cách reflective..." \
  --platform tiktok \
  --language vi \
  --target-duration 45

python skills/video-job-manager/scripts/manage_job.py register-input \
  --job jobs/2026-04-25_001_quang-cao-thoi-quen-buoi-sang \
  --kind raw_assets \
  --path /path/to/clip.mp4 \
  --copy

python skills/video-job-manager/scripts/manage_job.py mark-stage \
  --job jobs/2026-04-25_001_quang-cao-thoi-quen-buoi-sang \
  --stage creative_plan \
  --status done \
  --output source/creative_plan.toml

python skills/video-job-manager/scripts/manage_job.py stale-from \
  --job jobs/2026-04-25_001_quang-cao-thoi-quen-buoi-sang \
  --stage voice \
  --reason "audio voice đã thay đổi"

python skills/video-job-manager/scripts/manage_job.py status \
  --job jobs/2026-04-25_001_quang-cao-thoi-quen-buoi-sang

python skills/video-job-manager/scripts/manage_job.py todo \
  --job jobs/2026-04-25_001_quang-cao-thoi-quen-buoi-sang
```
...
```toml
[job]
id = "2026-04-25_001_quang-cao-thoi-quen-buoi-sang"
title = "Quảng cáo thói quen buổi sáng"
status = "created"
