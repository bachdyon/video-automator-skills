...
from remotion_deps import link_shared_node_modules
...
def strip_vietnamese_marks(value: str) -> str:
    """Return ASCII text while preserving Vietnamese d/D for slug generation."""
...
def slugify(value: str) -> str:
    text = strip_vietnamese_marks(value).lower()
    text = re.sub(r"[^a-z0-9]+", "-", text).strip("-")
    return text[:48].strip("-") or "video-job"


def validate_manual_job_id(value: str) -> str:
    if value != strip_vietnamese_marks(value):
        die("--job-id must be Vietnamese without accents; omit --job-id to derive it from --title")
    if not re.fullmatch(r"[a-z0-9][a-z0-9_-]*", value):
        die("--job-id must use lowercase ASCII letters, numbers, hyphens, or underscores only")
    return value
...
def create(args: argparse.Namespace) -> None:
    ...
    job_id = validate_manual_job_id(args.job_id) if args.job_id else next_job_id(root, args.title)
    path = root / job_id
    if path.exists():
    ...
    link_shared_node_modules(path / "remotion", Path(__file__).resolve().parents[3])
    created_at = now_iso()
    data = {
        "job": {
            "id": job_id,
