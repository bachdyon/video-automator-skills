...
4. **Dừng để user duyệt voiceover script** trước khi handoff sang voice, ảnh, transcript, mapping, render plan hoặc render. Lưu script vào `jobs/<job_id>/source/narration.txt` nếu đang ở trong job.
5. Sau khi user duyệt rõ ràng, ghi `jobs/<job_id>/source/voiceover_approval.toml` với `approved = true`, rồi mới tiếp tục.
6. Chia kịch bản thành scene_intents — không phải lựa chọn asset cuối cùng.
7. Định nghĩa overlay text, hành vi subtitle và CTA.
8. Định nghĩa asset_requirements để `asset-semantic-extractor` và `semantic-asset-mapper` biết tìm gì.
9. Giữ identifier, dữ liệu cá nhân, chi tiết riêng tư ở mức trừu tượng trừ khi user xác nhận sở hữu và yêu cầu rõ.

## Voiceover Approval Gate (BẮT BUỘC)

- Mọi creative plan có `[voiceover].script` dùng để sản xuất video phải được user duyệt trước khi chạy các bước downstream.
- Agent phải gửi nguyên văn script cho user và hỏi duyệt/chỉnh.
- Không chạy TTS/voice, không tạo ảnh AI, không transcribe, không map asset, không build render plan, và không render trước khi user xác nhận rõ ràng.
- Nếu user sửa script, cập nhật `creative_plan.toml`/`narration.txt`, hỏi duyệt lại, và đánh dấu mọi audio/transcript/render phụ thuộc script cũ là stale.
