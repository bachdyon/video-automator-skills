...
- Với video Lão Bách Niên, khi user yêu cầu giọng `Binh` hoặc `Thanh Bình`, dùng Free TTS preset `--voice-id Binh` trừ khi `.shared` có cấu hình voice clone riêng được user chỉ định rõ.
...
Với script dài, đặc biệt khi dùng `--mode standard`, thêm `--chunk-by-paragraph` để infer từng đoạn trong cùng một process rồi nối WAV, tránh một inference quá dài bị chậm hoặc treo:
