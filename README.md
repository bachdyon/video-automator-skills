...
- `$capcut-tts` — dùng CapCut common task client để tạo/query TTS task hoặc chạy STT/subtitle recognition qua flow CapCut không chính thức. [Chi tiết](https://vas.bachdyon.com/skills/capcut-tts)
...
- `$filepost-file-upload` — upload file lên FilePost, lấy URL CDN cố định; list/get/delete qua client Python. [Chi tiết](https://vas.bachdyon.com/skills/filepost-file-upload)
- `$heygen-asset-upload` — upload ảnh/video/audio/PDF lên HeyGen và nhận `asset_id` tái sử dụng. [Chi tiết](https://vas.bachdyon.com/skills/heygen-asset-upload)
- `$heygen-photo-avatar-video` — tạo video nhân hiệu / talking-head từ một ảnh người + audio qua HeyGen Image to Video. [Chi tiết](https://vas.bachdyon.com/skills/heygen-photo-avatar-video)
- `$video-compress-under-25mb` — nén MP4/MOV H.264/AAC để dưới ~25MB (giới hạn upload Zernio, inbox, v.v.). [Chi tiết](https://vas.bachdyon.com/skills/video-compress-under-25mb)
...
- `$subtitle-punch-tag-shortform` — caption short-form normal + PUNCH đồng bộ word-level (~7–8 từ/chunk), punch do agent suy luận ngữ nghĩa. [Chi tiết](https://vas.bachdyon.com/skills/subtitle-punch-tag-shortform)
...
- `$typography-style-selector` — chọn font direction, pairing, scale và weight contrast trước khi code/render visual nặng typography. [Chi tiết](https://vas.bachdyon.com/skills/typography-style-selector)
- `$video-visual-aesthetics` — khóa visual brief cho video render bằng HTML/CSS/JS hoặc Remotion: composition, motion, background, safe area và anti-template choices. [Chi tiết](https://vas.bachdyon.com/skills/video-visual-aesthetics)
- `$animated-svg` — biến SVG local thành HTML animation độc lập, giữ nguyên style gốc và chọn motion theo nội dung hình. [Chi tiết](https://vas.bachdyon.com/skills/animated-svg)
